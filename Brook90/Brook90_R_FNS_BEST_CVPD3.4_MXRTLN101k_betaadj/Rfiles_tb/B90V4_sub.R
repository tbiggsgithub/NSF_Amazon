...
# net radiation
  ...
  avai<-AVAILEN(SLRAD, ALBEDO, C1, C2, C3, TAJ, EA, SOVERI, SHEAT, CR, LAI, SAI)  # AVAILEN is in SUN.R
    RN<<-unlist(avai[1])  #  added by tbiggs to get out net radiation
    AA<<-unlist(avai[2])
    ASUBS<<-unlist(avai[3])
    LNGNET<<-unlist(avai[4])  #  added by tbiggs
    SLRADOUT <<-unlist(avai[5])  #  added by tbiggs.  Happens for both day and nighttime
    CLDCORLW <<- unlist(avai[6])  # only for my plotting purposes.
    EFFEMLW <<- unlist(avai[7])  # added by tbiggs by returning additional parameters in AVAILEN (in SUN.R)
# vapor pressure deficit
  esat<-ESAT(TAJ, ES, DELTA)
    ES<<-unlist(esat[1])
    DELTA<<-unlist(esat[2])
    VPD <<- ES - EA
    if (VPD<0) {VPD=0}  # Added by Tbiggs.
# S.-W. resistances
  swgra<-SWGRA(UAJ, ZA, HEIGHT, Z0, DISP, Z0C, DISPC, Z0GS, LWIDTH, RHOTP, NN, LAI, SAI, RAA, RAC, RAS)
    RAA<<-unlist(swgra[1])  # atmospheric aerodynamic resistance
    RAC<<-unlist(swgra[2])  #  canopy resistance
    RAS<<-unlist(swgra[3])  #  soil resistance
  if (J == 1) {
    RSClist <<- SRSC(SLRAD, TA, VPD, LAI, SAI, GLMIN, GLMAX, R5, CVPD, RM, CR, TL, T1, T2, TH)
    RSC <<- unlist(RSClist[1])  # TBiggs modified SRSC function to output resistance factors.
    FRINT <<- unlist(RSClist[2])  # PAR limitation.  Note that the list above is Input (not output)
    FD <<- unlist(RSClist[3])   #  VPD limitation
    FT <<- unlist(RSClist[4])   #  Temperature limitation
    fR <<- unlist(RSClist[5])   #  Radiation limitation
  }else{
    RSC <<- 1 / (GLMIN * LAI)
  }
# S.-W. potential transpiration and ground evaporation rates
  swpe<-  SWPE(AA, ASUBS, VPD, RAA, RAC, RAS, RSC, RSS, DELTA)
    PTR[J]<<-unlist(swpe[1])
    GER[J]<<-unlist(swpe[2])
    RNOUT[J]<<-RN  #  added by tbiggs--outputs RN for day (J=1) or night (J=2)
    SLRADOUTVEC[J]<<-SLRADOUT
# S.-W. potential interception and ground evap. rates
# RSC = 0, RSS not changed
  swpe<-  SWPE(AA, ASUBS, VPD, RAA, RAC, RAS, 0, RSS, DELTA)
    PIR[J]<<-unlist(swpe[1])
    GIR[J]<<-unlist(swpe[2])  # Potential ground evaporation rate when canopy wet
# actual transpiration and ground evaporation rates
  if (PTR[J] > 0.001) {
      ...
      ATR[J]<<-unlist(rbl[1])  # Total actual transpiration all layers
      ATRANI<<-unlist(rbl[2])  # Actual transpiration by layer
    for (i in 1:NLAYER){
      ATRI[J,i] <<- ATRANI[i]
    }
    if (ATR[J] < PTR[J]){
    # soil water limitation.  GER is ground evaporation rate 
    ...
    GER[J]<<-SWGE(AA, ASUBS, VPD, RAA, RAS, RSS, DELTA, 0, GER[J])  # GER is ground evaporation rate
  }
}
}
